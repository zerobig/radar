...
Перем ИменаМетодовОтменыТранзакции;
...
Процедура Открыть(Парсер, Параметры) Экспорт
	...
	ИменаМетодовОтменыТранзакции = Новый Структура;
	ИменаМетодовОтменыТранзакции.Вставить("отменитьтранзакцию");
	ИменаМетодовОтменыТранзакции.Вставить("rollbackransaction");
КонецПроцедуры
...
Процедура ПосетитьВыражениеИдентификатор(ВыражениеИдентификатор) Экспорт
	...
	Если Не ИменаМетодовОтменыТранзакции.Свойство(НРег(ВыражениеИдентификатор.Голова.Имя)) Тогда
		Возврат;
	КонецЕсли;
	
	Если Стек.Количество() > 1 И Не Стек[Стек.ВГраница() - 1].Тип = Типы.ОператорИсключение Тогда
		Ошибка("Вызов ""ОтменитьТранзакцию()"" находится вне конструкции ""Попытка... Исключение""",
